...
Create a static HTML file from an Agent Markdown document:
...
The generated HTML embeds the resolved document data for offline viewing. Referenced local artifacts are linked or shown with a fallback unless their content is already represented in the resolved document payload.
...
Use `::embed` to attach local generated artifacts to a report. In the browser viewer, Markdown, text, CSV, Mermaid, JSON, images, and videos can render as controlled previews through the local artifact endpoint; PDFs, HTML, and unsupported artifact types open as links instead of executing inline. In the VSCode/Cursor preview, embeds keep the same card, title, caption, and safe fallback without widening webview filesystem access.
